...
def main(argv=None):
    ...
    model_names = [join('style_model', f) for f in listdir('style_model') if isfile(join('style_model', f)) & ~f.lower().endswith('meta')]
    ...
    for i in range(len(model_names)):
        saver.restore(sess, model_names[i])
        tag = model_names[i][model_names[i].find('-') + 1:model_names[i].find('.')]
        print "load style: " + tag
        for j in tqdm.tqdm(xrange(total_batch)):
            ...
            scipy.misc.imsave('coco_style/%s-%s.png' % (content_name[0][5:-4], tag), output_t[0])

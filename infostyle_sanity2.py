...
def get_image(path, size, epochs, shuffle=False):
    ...
    return processed_image, img_key


def train_infogan():

    images, _ = get_image('/home/jump/coco_style', [128] + [128], 1, True)
    real_in = tf.train.batch([images], batch_size, dynamic_pad=True)
    ...
    d_loss, g_loss, q_loss, Gz = tower_loss(real_in, gram_in, Z_s)
...
def tower_loss(real_in, gram_in, zs):
    ...
    Dx, _, _ = gram_util.discriminator(real_in)
